...
class SshExperimentSubagent(ExperimentSubagent):
    ...
    def _build_launch_command(self, blueprint_path: str, session_name: str) -> str:
        ...
        return dedent(f"""\
            ...
            sleep 2 && \\
            tmux send-keys -t {session_name} "python -m alpha_auto_research.opencode_runner {Role.WORKER.value} --runner={Runner.SSH.value} --blueprint={blueprint_path}; rm -f {still_training}" Enter && \\
